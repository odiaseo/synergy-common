...
use SynergyCommon\Util\Inflector;
...
class Util
{
            ...
            array(
                'filters' => array(
                    ...
                    array(
                        'name'    => 'pregReplace',
                        'options' => array(
                            'pattern'     => '/\b(nl\/be|nl\/de|esp|pt|AR|AUS|llc|codes|dhs|gb|Smb|of|\(.*\)|\[.*\]|usa|ireland|payg|contracts|gmbh|eu|and|limited|ltd|plc|\.co\.|uk|inc|hu|ch|fr|es|nz|dk|se|ru|br|cn|jp|no|ca|ie|tr|au|lt|fi|other|dach|-uk|[^a-z0-9\-\_\s])\b/i',
                            'replacement' => '',
                        ),
                    ),
                    array(
     ...
     * @return string
     */
    public static function urlizeOld($data)
    {
        $title = self::prepareTitleForSlug($data);
        if (!$return = Urlizer::urlize($title)) {
            $return = Inflector::urlize($title);
        }

        $return = $return ?: Urlizer::transliterate($title);
